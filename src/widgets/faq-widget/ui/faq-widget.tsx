...
import { Accordion, Heading } from 'neobank-ui-kit';
...
const issuingFaq = [
  ...
  { title: 'In what currency can I issue a card?', text: 'In rubles, dollars or euro' },
  {
...
export const FaqWidget = () => {
  const [issuingFaqActiveIndex, setIssuingFaqActiveIndex] = useState(-1);
  const [usingFaqActiveIndex, setUsingFaqActiveIndex] = useState(-1);

  return (
    <section>
      <Heading style={{ fontWeight: 700, marginBottom: 32 }} level={3}>
        Issuing and receiving a card
      </Heading>
      <div>
        {issuingFaq.map((item, index) => (
          <Accordion
            key={`${item.title}_${item.text}`}
            text={item.text}
            title={item.title}
            index={index}
            setOpenedIndex={setIssuingFaqActiveIndex}
            isOpen={index === issuingFaqActiveIndex}
          />
        ))}
      </div>
      <Heading style={{ fontWeight: 700, marginBlock: 32 }} level={3}>
        Using a credit card
      </Heading>
      <div>
        {usingFaq.map((item, index) => (
          <Accordion
            key={`${item.title}_${item.text}`}
            text={item.text}
            title={item.title}
            index={index}
            setOpenedIndex={setUsingFaqActiveIndex}
            isOpen={index === usingFaqActiveIndex}
          />
        ))}
      </div>
    </section>
